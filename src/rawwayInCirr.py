...
def cirr_generate_test_dicts(relative_test_dataset: CIRRDataset, clip_model: CLIP, index_features: torch.Tensor,
    ...
    predicted_features, ref_features, reference_names, pairs_id, group_members = \
    ...
    if args.use_momentum_strategy:
        ...
        #         blip_predicted_features_list = []
        #         for i in range(nums_caption):
        #             blip_predicted_features, _ = cirr_generate_test_predictions(clip_model, relative_test_dataset, True, debiased_id=i)
        ...
        if args.use_debiased_sample:
            ...
            for i in range(len(predicted_features)):
                gpt_features = predicted_features[i]
                blip_features = ref_features[i]
            ...
            predicted_features_tensor = torch.stack(predicted_features)
            if 0:
...
def cirr_generate_test_predictions(clip_model: CLIP, relative_test_dataset: CIRRDataset,
    ...
    for batch in tqdm(relative_test_loader):
        ...
        if args.multi_caption:
            ...
            for cap in ref_captions:
                tokenized_ref_captions = tokenizer(cap, context_length=77).to(device)
                ref_features = clip_model.encode_text(tokenized_ref_captions)
                ref_features_list.append(text_features)
            text_features_list = torch.stack(text_features_list)
            text_features = torch.mean(text_features_list, dim=0)
            ref_features_list = torch.stack(text_features_list)
            ref_features = torch.mean(text_features_list, dim=0)
        ...
        ref_features = F.normalize(ref_features)
    ...
    ref_features = torch.vstack(ref_features_list)

    return predicted_features, ref_features , reference_names_list, pair_id_list, group_members_list
